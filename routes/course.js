...
router.post("/create", verifyToken ,upload.any(),  async function(req, res, next){
  ...
  jwt.verify(req.token, 'mysecretkey', async(err, authData) => {
    if(!err){
      const url = `http://159.65.216.209:3000/public/uploads/images/courses/${req.files[0].filename}`
      const captionUrl = `http://159.65.216.209:3000/public/uploads/images/courses/${req.files[1].filename}`
